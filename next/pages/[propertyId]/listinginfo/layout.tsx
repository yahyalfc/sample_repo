...
import AppNavigationComponent from '../../../../components/property-details-layout/MainLayout/AppNavigationComponent';
import TabNavigationBarComponent from '../../../../components/property-details-layout/MainLayout/TabNavigationBarComponent';
import PropertyImageComponent from '../../../../components/property-details-layout/MainLayout/PropertyImageComponent';
import NavigationBar from '../../../../components/property-details-layout/ListingInfo/components/UtilComponents/NavigationBar';
import { useSelector } from 'react-redux';
import { useRouter } from 'next/router';

const ListingLayout = ({ children, propertyId }) => {
...
// Find the index of "listinginfo" in the URL parts
const listingInfoIndex = parts.indexOf('listinginfo');
listingInfoIndex !== -1 && listingInfoIndex + 1 === parts.length - 1
  return (
    <div className='flex flex-row'>
      <AppNavigationComponent />
      <div className='w-full md:pl-[276px] pr-6 py-4 '>
        <PropertyImageComponent propertyId={propertyId} totalSteps={26} stepsCompleted={10} />
        <TabNavigationBarComponent propertyId={propertyId} />
        <div className='py-8 rounded-b-lg bg-white'>
        <div className={listingInfoIndex !== -1 && listingInfoIndex + 1 === parts.length - 1 ?'':'hidden'}>

        <NavigationBar propertyId={propertyId} currentCard={currentCard} onFrameContainerClick={onFrameContainerClick} setCurrentCard={setCurrentCard}/>
        </div>
          {children}
        <div className={"flex flex-row items-end px-6 justify-end gap-[12px]"}>
          <button className="cursor-pointer py-[9px] px-4 bg-white w-[85px] rounded-md box-border overflow-hidden shrink-0 flex flex-row items-start justify-start border-[1px] border-solid border-gg-primary-new hover:bg-gainsboro-100 hover:box-border hover:border-[1px] hover:border-solid hover:border-teal-100">
            <div className="relative text-sm leading-[20px] font-medium font-poppins text-gg-primary-new text-left inline-block min-w-[51px]">
              Cancel
            </div>
          </button>
          <button className="cursor-pointer [border:none] py-[9px] px-[22.5px] bg-gg-primary-new rounded-md shadow-[0px_1px_2px_rgba(0,_0,_0,_0.05)] overflow-hidden flex flex-row items-start justify-start whitespace-nowrap hover:bg-teal-100">
            <div className="relative text-sm leading-[20px] font-medium font-poppins text-white text-left inline-block min-w-[73px]">
              Save Data
            </div>
          </button>
        </div>
