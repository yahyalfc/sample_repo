...
export const extractFunctions: any = {
    ...
    extractFaqs(faqs: any, transformedObject:PropertySchema){
    ...
    extractAmenities(amenities: any, tranformedObject: PropertySchema){
    const tranformedAmenitites = amenities.map(amenity => amenity.name);
    return tranformedAmenitites
    },
    extractPropertyImage(images: any, tranformedObject: PropertySchema){
        return images?.length ? images[0]?.full_size_url : ""
    }
