...
export function transformTokeet(tokeet_object: any) {    
    console.log('ooooooooooooo');
    console.log({tokeet_object});
    console.log('ooooooooooooo');
    ...
    for (const [key, value] of Object.entries(mapping)) {
        if (Array.isArray(value)) {
            if (value.every(item => typeof item === 'string')) {
                ...
                transformedObject[key] = extractFunctions[extractor](tokeet_object[parameter], transformedObject);
                ...
                value.forEach((obj: any) => {
                    ...
                    if (Array.isArray(parameters)) {
                        ...
                        console.log("params", params) // Cast parameters to string[]
                        transformedObject[key] = extractFunctions[extractor](...params.map(param => tokeet_object[param]), transformedObject);
                    } else {
                        const param = parameters as string; // Cast parameters to string
                        transformedObject[key] = extractFunctions[extractor](tokeet_object[param], transformedObject);
                    }
                });
            }
        } else if (typeof value === 'string') {
            const transformedValue = getValueFromPath(tokeet_object, value);
            ...
            if (Array.isArray(parameters)) {
                ...
                // transformedObject[key] = extractFunctions[extractor](...params.map(param => tokeet_object[param]), transformedObject);
                assignValue(transformedObject, key, extractFunctions[extractor](...params.map(param => getValueFromPath(tokeet_object, param)), transformedObject))
            } else {
                const param = parameters as string; // Cast parameters to string
                transformedObject[key] = extractFunctions[extractor](tokeet_object[param], transformedObject);
            }
    ...
    console.log("Final Object", JSON.stringify(transformedObject, null, 3))
