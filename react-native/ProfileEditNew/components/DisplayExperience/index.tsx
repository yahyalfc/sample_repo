...
function DisplayProfileExperience({ experiences }: any) {

	const navigation = useNavigation();
	const onEdit = (index: number) => {
		navigation.navigate('CandidateEditExperienceScreen', { whichEducationOrExperience: index });
	}

	const onAdd = () => {
		navigation.navigate('CandidateAddExperienceScreen');
	}
	...
	return (
		<View style={{ width: "100%" }}>
			<View style={{
				...
				<View>
					<Text style={{ fontFamily: "Lato-Regular", textAlign: 'left', fontSize: 20, fontWeight: "600", color: "#012653", marginBottom: experiences.length ? 15 : 0 }}>Experience</Text>
					<View style={{ paddingTop: 7, paddingBottom: 7, }}>
						{experiences?.length ? experiences.map((val: any, index: number) => {
							return (
										...
										<View style={{ paddingLeft: 20, width: '100%' }}>
											{val?.current == true && val?.startYear ?
												...
												!!val?.startYear && !!val?.endYear ?
													...
													: null}
											{!!val?.position &&
												<Text style={{ marginBottom: 3, fontFamily: 'Lato-Regular', fontWeight: '700', fontSize: 16, color: "rgba(0, 0, 0, 0.85)", width: '90%' }} numberOfLines={1}>{val?.position}</Text>}
											<View style={{ display: 'flex', flexDirection: 'row', width: '100%', alignItems: 'flex-end' }}>
												{val?.company ?
													<Text style={{ fontFamily: 'Lato-Regular', fontWeight: '700', fontSize: 14, color: "rgba(0, 0, 0, 0.85)", maxWidth: wp("45%"), }} numberOfLines={1}>@{val?.company}</Text>
													: null}
												{!!val?.location ?
													<Text style={{ width: Platform.OS === "ios" ? "35%" : "45%", fontFamily: 'Lato-Regular', fontWeight: '400', fontSize: 14, color: "rgba(0, 0, 0, 0.85)", paddingTop: 3 }} numberOfLines={1}> | {val?.location?.city},{val?.location?.country}</Text>
													: null}
											</View>
										</View>
									</View>
					...
					<View style={{ display: "flex", flexDirection: "row", alignItems: "center" }} >
						...
						>
							<View style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
								...
								<Text style={{ color: "#1D97A2", fontFamily: 'Lato-Regular', fontWeight: '500', fontSize: 18 }}>Add a new experience</Text>
							</View>
						</TouchableOpacity>
					</View>
				</View>
...
export default DisplayProfileExperience
